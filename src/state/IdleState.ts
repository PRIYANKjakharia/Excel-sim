...
export class IdleState implements GridState {
    private handlers: InteractionHandler[] = [new RowResizeHandler(), new ColumnResizeHandler(),new RowSelectionHandler(), new ColumnSelectionHandler(), new CellSelectionHandler() ];

    public onPointerDown(grid: Grid, event: PointerEvent): void {
        grid.getEditor().hide();
        grid.getCanvas().focus();

        if ( event.offsetX < GridConfig.HEADER_WIDTH &&event.offsetY < GridConfig.HEADER_HEIGHT ){
            grid.getSelection().selectAll();
            grid.render();
            return;
        }
