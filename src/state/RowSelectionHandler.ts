...
export class RowSelectionHandler implements InteractionHandler, GridState {
    ...
    public isMatch(grid: Grid, event: PointerEvent): boolean {
        // Clicked within left row bar bounds but NOT on a split line
        return event.offsetX < GridConfig.HEADER_WIDTH && event.offsetY >= GridConfig.HEADER_HEIGHT && GeometryHelpers.getRowResizeIndex(grid, event.offsetY) === null;
    }
